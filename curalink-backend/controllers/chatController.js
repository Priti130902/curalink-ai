...
exports.handleChat = async (req, res) => {
  ...
  try {
    const [pubmed, trials] = await Promise.all([
      axios.get(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${expandedQuery}&retmax=30&retmode=json`),
      axios.get(`https://clinicaltrials.gov/api/v2/studies?query.cond=${disease}&pageSize=30&format=json`)
    ]);

    const candidatePool = [
      ...(pubmed.data.esearchresult.idlist || []).map(id => ({
        title: `PubMed Analysis: ID ${id}`, source: 'PubMed', url: `https://pubmed.ncbi.nlm.nih.gov/${id}/`, year: 2026
      })),
      ...(trials.data.studies || []).map(s => ({
        title: s.protocolSection.identificationModule.officialTitle || "Clinical Trial",
        source: 'ClinicalTrials.gov', url: `https://clinicaltrials.gov/study/${s.protocolSection.identificationModule.nctId}`, year: 2026
      }))
    ];
